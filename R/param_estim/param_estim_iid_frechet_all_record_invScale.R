...
Plot_auto = function(variable_name, matrice = sum_matrices, file_name = "plot_bias.png"){
  ...
  A_bias <- sapply(matrice, function(x) x[variable_name, "shape"])
  ...
  # legend("topright", legend = c("Power", "1/shape", "scale"), col = c("plum", "cyan4", "lightgreen"), lty = 1, pch = 1, cex = 0.7)
...
## file name if saving
file_name = paste0("Simulation_iid_Xt_Rn_overT_frechet_scale=",scale_obs_inv,"_shape=", shape_obs,"_simulation_",simulation, ".xlsx")
out_path = "data/param_est/iid/frechet_invScale/"
###########################  Simulation  ###########################
...
for (T_val in T_values) {
  ...
  colnames(sum_matrix_record) = c("gamma","shape","scale","N_T")
  ...
  while(length(no)>=1){ #while (sim <= simulation){
    for(sim in no){
      ...
      y=VGAM::rfrechet(n=T_val,scale=scale_obs,shape = shape_obs)
      R=rec_values(y)
      L = rec_times(y)
      ...
      while(length(R)<=2) {
        ...
        logLik_fun<- loglik_registry[["iid"]][["all"]][["frechet_inv_scale"]]
        ...
        ## Records Data
        logLik_fun_rec <- loglik_registry[["iid"]][["records"]][["frechet_inv_scale"]]
        ...
        MLE_C_record = c(MLE_C_record$par, "objective" = - MLE_C_record$objective ) ## return shape, scale, shape, objective value
      ...
      ### Empirical variance
        ...
        emp_var = var_fun(data = y, params=list(scale = MLE_C["scale"], shape = MLE_C["shape"] ))
        Emp_var[sim,1] = 1
        Emp_var[sim, "shape"] = emp_var$var_shape
        Emp_var[sim, "scale"] = emp_var$var_scale

        theo_var = var_fun(data = y, params=list(scale = scale_obs_inv, shape = shape_obs ))
        Theo_var[sim,1] =1
        Theo_var[sim,"shape"] = theo_var$var_shape
        Theo_var[sim,"scale"] = theo_var$var_scale

        ## Record data
        var_fun = var_logLik_records("iid", "records","frechet_inv_scale", "all")

        emp_var = var_fun(data = data_rec, params=list(scale = MLE_C["scale"], shape = MLE_C["shape"] ))
        Emp_var_record[sim,1] = 1
        Emp_var_record[sim, "shape"] = emp_var$var_shape
        Emp_var_record[sim, "scale"] = emp_var$var_scale

        theo_var = var_fun(data = data_rec, params=list(scale = scale_obs_inv, shape = shape_obs ))
        Theo_var_record[sim,1] =1
        Theo_var_record[sim,"shape"] = theo_var$var_shape
  ...
  sum_matrix[2, ] <- round(colMeans(params) - c(gamma, shape_obs, scale_obs_inv, 0), 5)  # Average bias
  ...
  sum_matrix_record[2, ] <- round(colMeans(params_record) - c(gamma, shape_obs, scale_obs_inv, 0), 5)  # Average bias
  ...
  sum_matrix[5, ] <- c(CP_gamma, CP_shape, CP_scale, 0)

  CP_gamma_record <- power_g_record / simulation
  CP_shape_record <- power_A_record / simulation
  CP_scale_record <- power_a_record / simulation
  sum_matrix_record[5, ] <- c(CP_gamma_record, CP_shape_record, CP_scale_record, 0)
