...
## Series length
#T_val=50
T_values <- c(50, 100, 150, 200)

# Define the sequence of gamma values
trend_values <- seq(1.1, 1.25, by = 0.05)
...
## file name if saving
file_name = paste0("Simulation_ynm_Xt_Rn_gumbel_location=",location_obs,"_scale=", scale_obs, "_1000.xlsx")
out_path = "data/param_est/ynm/gumbel/"
...
for (T_val in T_values) {
  ...
  for (trend in trend_values) {
    ...
    colnames(params_record) <- c("gamma", "scale", "location", "N_T")

    LogL_record <- matrix(0, nrow = simulation, ncol = 1)

    Emp_var_record <- matrix(0, nrow = simulation, ncol = 3)
    colnames(Emp_var_record) <- c("gamma", "scale", "location")

    Theo_var_record <- matrix(0, nrow = simulation, ncol = 3)
    colnames(Theo_var_record) <- c("gamma", "scale", "location")
    ...
    colnames(sum_matrix_record) = c("gamma","scale","location","N_T")

    conf_int_record <- matrix(0, nrow = simulation, ncol = 6)
    colnames(conf_int_record) <- c("g_L", "g_U", "A_L", "A_U", "a_L", "a_U")

    ##__________________________ Simulation Code __________________________
    ...
    while(length(no)>=1){ #while (sim <= simulation){
      for(sim in no){
        ...
        ## generate ynm series of gumbel underlying distr
        y= YNM_series(T = T_val,gamma = trend, dist = "gumbel",
        ...
        ## All data
        logLik_fun <- loglik_registry[["YNM"]][["all"]][["gumbel_explicit"]]
        MLE_C = estimate_model_mle(logLik_fun, obs_type = "all", data = y,
                                   lower_bounds=c(gamma=1.01, location = 0.01, scale=0.01),
                                   upper_bounds = c(gamma=10, location = 10, scale=10),
                                   start_values = c(gamma=1.01, location = 0.01, scale=0.01)
        )
        MLE_C = c(MLE_C$par, "objective" = - MLE_C$objective )

        ## Records Data
        logLik_fun_rec <- loglik_registry[["YNM"]][["records"]][["gumbel_explicit"]]
        ...
        MLE_C_record = estimate_model_mle(logLik_fun_rec, obs_type = "records", data = data_rec,
                                         lower_bounds=c(gamma=1.001, location = 0.01, scale=0.01),
                                         upper_bounds =c( 100, location =10, scale=100),
                                         start_values = c(
                                           gamma = 1.01,
                                           location = 0.01,
                                           scale =0.01
        ...
        theo = var_fun(data = y, params=list(gamma = trend, scale = scale_obs, location = location_obs ) )
        #compute_var_hessian(loglik_fun = logLik_fun, y, par_vec = list(gamma = trend, scale = scale_obs, location = location_obs ), eps = 1e-6 )
        ...
        theo = var_fun(data = data_rec, params=list(gamma = trend, scale = scale_obs, location = location_obs ) )
        #compute_var_hessian(loglik_fun = logLik_fun_rec, data_rec, par_vec = list(gamma = trend, scale = scale_obs, location = location_obs ), eps = 1e-6 )
...
# df_long <- df_param %>%
#   tidyr::pivot_longer(cols = c(gamma, scale, location),
#                names_to = "Parameter",
#                values_to = "Value")
#
...
plots <- plot_extremes_color(
  final_df,
  obs_col = "gamma_obs",
  param_cols = c("gamma", "scale", "location")
)
