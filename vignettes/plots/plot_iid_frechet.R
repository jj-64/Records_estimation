...
library(tidyr)
library(ggplot2)
path_out = "vignettes/plots_output/iid/frechet_invScale/"
#___________________________________
# Read data ---------
#___________________________________

df <- read_excel("data/param_est/iid//frechet_invScale/Simulation_iid_Xt_Rn_overT_frechet_scale=1_shape=2_simulation_5000.xlsx", sheet = "All Par") %>% mutate(Scenario = "All observations")
df_record <- read_excel("data/param_est/iid/frechet_invScale/Simulation_iid_Xt_Rn_overT_frechet_scale=1_shape=2_simulation_5000.xlsx", sheet = "All Par_record")%>% mutate(Scenario = "Records")
...
bias_df <- df_long %>%
  ...
  pivot_longer(
    cols = c(AVG_bias_shape, AVG_bias_scale),
    names_to = "Parameter",
    values_to = "Bias"
  ) %>%
  mutate(
...
var_df <- df_long %>%
  ...
  mutate(
    Parameter = ifelse(grepl("shape", Variable),
                       "Shape",
                       "Scale"),
...
rmse_data <- bind_rows(
  df %>% mutate(
                ...
                RMSE_shape = AVG_bias_shape^2 + AVG_Emp_var_shape),
  df_record %>% mutate(
                       RMSE_scale = AVG_bias_scale^2 + AVG_Emp_var_scale,
                       RMSE_shape = AVG_bias_shape^2 + AVG_Emp_var_shape)
) %>%
...
ggsave(paste0(path_out,"iid_frechet.pdf"),
       combined_plot,
       width = 9,
       height = 9,
       device = cairo_pdf)

ggsave(paste0(path_out,"iid_frechet.png"),
       combined_plot,
       width = 12,
       height = 9,
       dpi = 300)
