...
library(patchwork)
library(scales)
path_out = "vignettes/plots_output/ldm/ldm_frechet_inv_scale/"
...
df_full <- read_excel(paste0("data/param_est/ldm/frechet_inv_scale/Simulation_ldm_Xt_Rn_frechet_shape=2_scale=1_simulation_",simulation,".xlsx"), sheet = "All par") %>% mutate(Scenario = "All observations")
df_records <- read_excel(paste0("data/param_est/ldm/frechet_inv_scale/Simulation_ldm_Xt_Rn_frechet_shape=2_scale=1_simulation_",simulation,".xlsx"), sheet = "All par_record")%>% mutate(Scenario = "Records")
...
rmse_data <- df %>% dplyr::select(-N_T) %>%
  ...
  pivot_wider(
    names_from = Metric,
    values_from = c(theta, scale, shape)
  ) %>%
  mutate(
...
ggsave(paste0(path_out,"ldm_frechet.pdf"),
       combined_plot,
       width = 9,
       height = 9,
       device = cairo_pdf)

ggsave(paste0(path_out,"ldm_frechet.png"),
       combined_plot,
       width = 12,
       height = 9,
       dpi = 300)
