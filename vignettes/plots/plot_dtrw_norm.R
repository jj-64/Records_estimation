...
library(ggplot2)
library(dplyr)
path_out = "vignettes/plots_output/dtrw/norm/"
...
df <- read_excel(paste0("data/param_est/dtrw/norm/Simulation_dtrw_Xt_Rn_overT_norm_sd=",true_param,".xlsx"), sheet = "All Par") %>% mutate(Scenario = "All observations")
df_record <- read_excel(paste0("data/param_est/dtrw/norm/Simulation_dtrw_Xt_Rn_overT_norm_sd=",true_param,".xlsx"), sheet = "All Par_record") %>% mutate(Scenario = "Records")
...
df_plot2 <- bind_rows(
  ...
  mutate(T_num = as.numeric(gsub("T=", "", T))) %>%
  pivot_longer(
    cols = c(AVG_Emp_var_sd, AVG_Theo_var_sd),
  ...
  mutate(
    Variance_Scenario = recode(
      Variance_Scenario,
      AVG_Emp_var_sd = "Empirical variance",
      AVG_Theo_var_sd = "Theoretical variance"
    ),
...
variance_plot <- ggplot(
  ...
  labs(
    ...
    y = "Variance",
    color = "Scenario",
    linetype = "Variance Scenario",
    shape = "Variance Scenario"
  )
...
rmse_data <-
  df_plot %>% mutate(
    RMSE_sd = AVG_bias_sd^2 + AVG_Emp_var_sd) %>%
  mutate(T_num = as.numeric(gsub("T=", "", T))) %>%
  pivot_longer(
    cols = c(RMSE_sd),
    names_to = "Parameter",
...
ggsave(paste0(path_out,"dtrw_norm.pdf"),
       combined_plot,
       width = 9,
       height = 9,
       device = cairo_pdf)

ggsave(paste0(path_out,"dtrw_norm.png"),
       combined_plot,
       width = 12,
       height = 9,
       dpi = 300)
